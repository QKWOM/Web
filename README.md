...
- 常用会议和期刊按研究方向分组（机器人、机器学习、计算机视觉、自然语言处理、人工智能、数据挖掘与数据库、图形学与人机交互、软件与安全），每组分会议、期刊两列；机器人方向包括 CoRL、ICRA、IROS、RSS 和 T-RO、IJRR、RA-L、Science Robotics、RAM、AURO、JFR、RAS
...
OpenReview 现在会对匿名的“列表”请求做人机验证：按会场列出论文（`/notes?content.venueid=…`）会返回“Challenge verification required”；按标题搜索单篇论文（`/notes/search`）目前不受影响，但无法用来列出整个会场。

- **默认不登录**：dblp 尚未收录的最近几年，会显示为带 ↗ 的年份按钮（例如 CoRL 2025），点击在 OpenReview 网站上打开该会场，在浏览器里查看录用论文。确认需要登录后，本次浏览不再反复请求 OpenReview。
- **想在网页里直接列出这些论文**：运行 `python3 server.py --login`，在终端输入 OpenReview 登录邮箱和密码（密码输入时不显示）。只在本机保存一个一周有效的登录令牌（项目目录下的 `.openreview_token`，仅当前用户可读，不会提交到 git），不保存密码；之后直接 `python3 server.py` 即可。
- 也可以用环境变量提供账号：`OPENREVIEW_USERNAME=邮箱 OPENREVIEW_PASSWORD=密码 python3 server.py`，这时令牌过期会自动重新登录。
- 退出登录：`python3 server.py --logout`。暂不支持开启了两步验证的账号。
